...
class TestContracts(unittest.TestCase):
    ...
    def test_investigation_case_v1_valid(self):
        fixture_path = Path(__file__).resolve().parent.parent.parent.parent / "fixtures" / "investigations" / "investigation-case-v1-valid.json"
    ...
    def test_scenario_001_m1_valid(self):
        fixture_path = Path(__file__).resolve().parent.parent.parent.parent / "fixtures" / "network_intelligence" / "network-intelligence-v1-m1-phase1.json"
        with open(fixture_path, 'r', encoding='utf-8') as f:
            valid_m1 = json.load(f)
        self.validator.validate("network-intelligence-v1.json", valid_m1)

    def test_scenario_001_m2_valid(self):
        fixture_path = Path(__file__).resolve().parent.parent.parent.parent / "fixtures" / "findings" / "finding-v1-scenario-001.json"
        with open(fixture_path, 'r', encoding='utf-8') as f:
            valid_m2 = json.load(f)
        self.validator.validate("finding-v1.json", valid_m2)

    def test_scenario_001_m3_expected_valid(self):
        fixture_path = Path(__file__).resolve().parent.parent.parent.parent / "fixtures" / "investigations" / "investigation-case-v1-scenario-001-expected.json"
        with open(fixture_path, 'r', encoding='utf-8') as f:
