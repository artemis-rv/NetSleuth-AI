...
class CaseAccessModel(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("identity.users.user_id", ondelete="CASCADE"), primary_key=True, nullable=False)
    case_id = Column(UUID(as_uuid=True), ForeignKey("investigation.investigation_cases.case_id", ondelete="CASCADE"), primary_key=True, nullable=False)
